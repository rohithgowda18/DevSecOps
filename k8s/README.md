...
## CI note (helpful hint from pipeline run screenshot)

You provided a screenshot showing a SonarCloud scan failing with a message like "You must define the following mandatory properties for 'order-service': sonar.organization". That's a CI configuration issue (not a Kubernetes runtime problem). Quick fixes:

- Add `sonar.organization` to the Sonar analysis configuration (pipeline env or `pom.xml` properties) or set the SonarCloud organization key in GitHub Actions secrets/inputs.
- Ensure the Sonar token and organization settings are present in the GitHub Actions workflow before running `mvn sonar:sonar`.

## Recommended production changes

- Push images to a container registry and reference fully-qualified image names in manifests.
- Use `replicas > 1` for the Order Service and add proper pod disruption budgets.
- Replace in-memory stores with a persistent datastore (RDBMS / NoSQL) if you need data durability.
- Add resource and HPA (Horizontal Pod Autoscaler) configuration based on metrics.
- Secure services with network policies or an API gateway, and use secrets (Kubernetes Secrets) for credentials.

---

If you want, I can:

- Update the manifests to reference registry images and change `imagePullPolicy`.
- Add a `k8s/deploy.sh` script that builds images, loads them into Minikube, and applies manifests.
- Add or adjust probes to use a simple HTTP health endpoint if Actuator is not enabled.

Tell me which change you'd like and I'll make it and run quick validations.
